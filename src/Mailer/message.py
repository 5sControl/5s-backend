...
def send_email_to_suppliers(item, image_path):
    ...
    try:
        ...
        supplier = Company.objects.get(id=item['suppliers'])

        if item['name'] is None:
            raise ValueError("Missing item name")
        if supplier.name_company is None:
            raise ValueError("Missing supplier name")
        if supplier.contact_email is None:
            raise ValueError("Missing supplier contact email")
